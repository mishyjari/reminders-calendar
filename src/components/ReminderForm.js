...
class ReminderForm extends Component {
  ...
  componentDidMount() {
    ...
    if (
      this.props.reminder &&
      this.props.reminder.date.isSame(this.props.day, 'day')
    ) {
  ...
  handleChangeTime = e => {
    this.setState(prevState => ({
      date: moment(
        `${prevState.date.format('YYYY-MM-DD')} ${e.target.value}`,
        'YYYY-MM-DD HH:mm'
      ),
    }));
  };
  handleChangeDate = e => {
    this.setState(prevState => ({
      date: moment(
        `${e.target.value} ${prevState.date.format('HH:mm')}`,
        'YYYY-MM-DD HH:mm'
      ),
    }));
  };
  ...
  render() {
    ...
    return (
      <form
        ...
        id={`new-reminder-form-${day.unix()}`}
        onSubmit={this.handleSubmit}
      >
        <h4 className='form-heading'>
          {this.state.id ? 'Edit Reminder' : 'Create Reminder'}
        </h4>
